...
exports.addSubAdmin = async (req, res) => {
    try {
        ...
        db.query(insertData, [username, fullname,address,phone,hash], (err, result) => {
            if (err) {
                res.send({ status: 400, message: "Something went wrong while Add sub admin Or use are using duplicate phone/userName!" });
...
exports.getSubAdminDetails = async (req, res) => {
    try {
        ...
        var query = `select * from sub_admin where username = '${username}' and is_delete = 0`;
